...
# Round 3 Escalation - Manager and Boss list together
Base = pd.DataFrame(df3['Country'].unique())
# Renaming columns in Base data
Base.rename(columns={ Base.columns[0]: "Country" }, inplace = True)
Base['Country'] = Base['Country'].str.upper()
# Fetching the data for Contact Defence List
Defense_List = pd.read_excel('Contact Defence List.xlsx', sheet_name="Contact Defence List")
Defense_List['Country'] = Defense_List['Country'].str.upper()
Vlookup_Data = pd.merge(Base, 
                      Defense_List[['Country','FM & TM BORM']], 
                      on ='Country', 
                      how ='left')

for b in Vlookup_Data['FM & TM BORM']:
    #print (b)
    BRM_List.append(b)
Escalation_List = Manager_List + BRM_List
print('Escalation_List:', Escalation_List)
print(" If NAN is present in Escalation list then the country is not in the Contact Defence List ")
...
#    Date = df2['Date'].iloc[0]
    ...
    # subject = 'AHA EOD email summary ' + " " + "-" + " "+ str(Id) + ''
        subject = 'FM Supervisory Attestation - Q* 2022 [INTERNAL] - R3'
        # Users
        ...
        mail.CC = ";".join(str(e)  for e in Escalation_List)
        mail.Sensitivity = 3
        result = df3.to_html()
        mail.HTMLBody = """
        <html>

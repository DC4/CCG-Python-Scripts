...
os.chdir("C://Users//1510806//OneDrive - Standard Chartered Bank//Desktop//Conduct//Group Mandatory ELearning mapping")
...
Raw_FMSWData = pd.read_excel(Raw_FMSWData, 'Group Mandatory E - Learning Ov')
...
output[['Employee PSID', 'Name', 'Dummy']] = Raw_FMSWData['Name'].str.split("-", expand = True)
output[['LM PSID', 'LM Name', 'Dummy1']] = Raw_FMSWData['LM Name'].str.split("-", expand = True)
output['Type of Breach'] = 'Group Mandatory e-Learning Overdue'
output['Sub categories'] = 'Compliance Risk'
output['Severity'] = Raw_FMSWData['MI RAG Status']
output['Accountability'] = 'NA'
output['Materiality'] = output['Type of Breach'] + ' ' + output['Severity']
output['Comment'] = Raw_FMSWData['E-Learning Name'] + ' ' + Raw_FMSWData['Ageing'].astype(str)
output['FMSW/non-FMSW'] = 'FMSW'


# output['Material?']
def Categorize_Severity(Materiality):
    if Materiality in ("Group Mandatory e-Learning Overdue RED"):
        return 'Material'
    elif Materiality in ("Group Mandatory e-Learning Overdue AMBER") or Materiality in ("Group Mandatory e-Learning Overdue GREEN"):
        return 'Non-Material'
    else:
        return 'Uncategorized'
    
output['Material?'] = output['Materiality'].apply(Categorize_Severity)
...
output.set_axis(['Quarter', 'Date', 'Employee PSID', 'Name', 'LM PSID',
       ...
       'Accountability', 'Materiality','Comment', 'FMSW/non-FMSW', 'Material?',
       'Location', 'Region', 'Business (Lvl 6)', 'Role', 'LM Location',
...
Group_Mandatory_ELearning_mapping_Output = output
Group_Mandatory_ELearning_mapping_Output.to_excel("Group_Mandatory_ELearning_mapping_Output.xlsx")

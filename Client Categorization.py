...
os.chdir("C://Users//1510806//OneDrive - Standard Chartered Bank//Desktop//Conduct//Client Categorization")
# Getting the Base Datasets
# Raw_FMSWData
Raw_FMSWData = pd.ExcelFile('C://Users//1510806//OneDrive - Standard Chartered Bank//Desktop//Conduct//Client Categorization//FMSW Client Categorization.xlsx')
Raw_FMSWData = pd.read_excel(Raw_FMSWData, 'FMSW Client Categorization')
...
output[['LM PSID', 'LM Name']] = Raw_FMSWData['Employee LM'].str.split("-", expand = True)
output['Location'] = Raw_FMSWData['Employee Country']
output['LM Location'] = Raw_FMSWData['Employee LM Country']
output['Type of Breach'] = 'Client Categorisation'
output['Sub categories'] = 'Compliance Risk'
output['Severity'] = "Rag - " + Raw_FMSWData['RAG in PDC Rule']
output['Accountability'] = Raw_FMSWData['Disciplinary Action Taken']
output['Materiality'] = output['Type of Breach'] + output['Severity']
output['Comment'] = Raw_FMSWData['FO Staff Justification']
...
def Categorize_Severity(Severity):
    if Severity in ("RED"):
        return 'Material'
    elif Severity in ("AMBER"):
        return 'Non-Material'
    else:
        return 'Uncategorized'
    
output['Material?'] = Raw_FMSWData['RAG in PDC Rule'].apply(Categorize_Severity)
...
output.set_axis(['Quarter', 'Date', 'Employee PSID', 'Name', 'LM PSID', 'LM Name',
       'Location_x', 'LM Location', 'Type of Breach', 'Sub categories',
       'Severity', 'Accountability', 'Materiality', 'Comment', 'FMSW/non-FMSW',
       'Material?', 'Location_y', 'Region', 'Business (Lvl 6)', 'Role'], axis='columns', inplace=True)
Stafflist_LM = Stafflist[['Bank Id', 'Staff Region']]
Stafflist_LM.set_axis(['LM PSID', 'LM Region'], axis='columns', inplace=True)
output['LM PSID'] = output['LM PSID'].astype(int)
output = pd.merge(output, Stafflist_LM, on="LM PSID", how="left")
cols = ['Location_y']
output = output.drop(cols, axis=1)
output.set_axis(['Quarter', 'Date', 'Employee PSID', 'Name', 'LM PSID', 'LM Name',
       'Location', 'LM Location', 'Type of Breach', 'Sub categories',
       'Severity', 'Accountability', 'Materiality', 'Comment', 'FMSW/non-FMSW',
       'Material?', 'Region', 'Business (Lvl 6)', 'Role',
...
Client_Categorization_Output = output
Client_Categorization_Output.to_excel("Client_Categorization_Output.xlsx")
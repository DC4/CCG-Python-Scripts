# -*- coding: utf-8 -*-
"""
Created on Fri Dec 20 12:19:41 2024
...
os.chdir("C://Users//1510806//OneDrive - Standard Chartered Bank//Desktop//Conduct//OBA - Non FMSW")

# Getting the Base Datasets
# OBA
OBA = pd.ExcelFile('C://Users//1510806//OneDrive - Standard Chartered Bank//Desktop//Conduct//OBA.xlsx')
OBA = pd.read_excel(OBA, 'Sheet1')
print("Shape of OBA:", OBA.shape)
# Mapping
...
output = pd.DataFrame()
# Fetching all data from OBA
output['Quarter'] = pd.PeriodIndex(OBA['Month of Close Date'], freq='Q')
output['Date'] = OBA['Month of Close Date']
output['Employee PSID'] = OBA['Staff ID']
output['Type of Breach'] = 'OBA'
output['Sub categories'] = 'Compliance Risk'
output['Severity'] = OBA['Final Severity Rating']
...
output['Comment'] = OBA['Breach Description']
...
output['Material?'] = OBA['Final Severity Rating'].apply(Categorize_Severity)
# output.to_excel("Output_Missed_Trade_Temporary.xlsx")

# output[['Employee PSID', 'Name', 'Dummy']] = OBA['PSID - Name'].str.split("-", expand = True)
# output['Location'] = OBA['Responsible User\'s Country']
...
OBA_Out = output
OBA_Out.to_excel("OBA_Output.xlsx")

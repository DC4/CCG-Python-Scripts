# -*- coding: utf-8 -*-
"""
Created on Fri Dec 20 11:59:17 2024
...
os.chdir("C://Users//1510806//OneDrive - Standard Chartered Bank//Desktop//Conduct//TCIW - Non FMSW")

# Getting the Base Datasets
# TCIW
TCIW = pd.ExcelFile('C://Users//1510806//OneDrive - Standard Chartered Bank//Desktop//Conduct//CCIB FM Consolidated Monthly Breaches Reports - New.xls')
TCIW = pd.read_excel(TCIW, 'Sheet1')
print("Shape of TCIW:", TCIW.shape)
# Mapping
...
output = pd.DataFrame()
# Fetching all data from TCIW
output['Quarter'] = pd.PeriodIndex(TCIW['Month of Close Date'], freq='Q')
output['Date'] = TCIW['Month of Close Date']
output['Employee PSID'] = TCIW['Staff ID']
output['Type of Breach'] = 'TCIW'
output['Sub categories'] = 'Compliance Risk'
output['Severity'] = TCIW['Final Severity Rating']
...
output['Comment'] = TCIW['Breach Description']
...
def Categorize_Severity(Severity):
    if Severity in ("Low | LOW"):
        return 'Non-Material'
    elif Severity in ("Medium | MEDIUM") or Severity in ("Hgh | HIGH"):
        return 'Material'
    else:
        return 'Uncategorized'
    
output['Material?'] = TCIW['Final Severity Rating'].apply(Categorize_Severity)
# output.to_excel("Output_Missed_Trade_Temporary.xlsx")

# output[['Employee PSID', 'Name', 'Dummy']] = TCIW['PSID - Name'].str.split("-", expand = True)
# output['Location'] = TCIW['Responsible User\'s Country']
...
TCIW_Out = output
TCIW_Out.to_excel("TCIW_Output.xlsx")

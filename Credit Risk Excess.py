...
os.chdir("C://Users//1510806//OneDrive - Standard Chartered Bank//Desktop//Conduct//Credit Risk Excess")
...
FMSWData_12 = pd.read_excel(Raw_FMSWData_12, 'Credit Risk Excess')
...
output[['Employee PSID', 'Name', 'Dummy']] = FMSWData_12['Dealer PSID - Name'].str.split("-", expand = True)
output['Location'] = FMSWData_12['Dealer Country']
output['Region'] = FMSWData_12['Dealer\'s Region']
output['Type of Breach'] = 'Credit Risk Excess'
...
output['Workflow Status'] = FMSWData_12['Workflow Status']
# Exclude "Workflow Status" from "Workflow Status"
output = output[~output['Workflow Status'].str.contains("Recall and Closed", na=False, case=False)]

# output['Materiality']
def Categorize_Materiality(Severity):
    if Severity in ("High", "high"):
        return 'Credit Risk Excess High'
    elif Severity in ("Medium", "medium"):
        return 'Credit Risk Excess Medium'
    elif Severity in ("Low", "low"):
        return 'Credit Risk Excess Low'
    else:
        return 'Uncategorized'

output['Materiality'] = output['Severity'].apply(Categorize_Materiality)
...
output['Comment'] = FMSWData_12['Explanation / Details of Control Break Down']
...
output.set_axis(['Quarter', 'Date', 'Employee PSID', 'Name', 'Location', 'Region',
       ...
       'Materiality', 'Material?', 'Comment', 'FMSW/non-FMSW', 'LM PSID',
       'LM Name', 'Business (Lvl 6)', 'Role'], axis='columns', inplace=True) 
...
Best_Ex_Output = output
Best_Ex_Output.to_excel("Credit_Risk_Excess_Output.xlsx")
